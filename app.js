...
function syncDOMToState() {
  state.days = Math.max(1, parseInt(daysInput.value.replace(/[^\d]/g, ''), 10) || 1);

  for (const [key, el] of Object.entries(variableInputs)) {
    state.variable[key] = parseFloat(el.value.replace(/[^\d.]/g, '')) || 0;
  }
  for (const [key, el] of Object.entries(fixedInputs)) {
    state.fixed[key] = parseFloat(el.value.replace(/[^\d.]/g, '')) || 0;
  }
